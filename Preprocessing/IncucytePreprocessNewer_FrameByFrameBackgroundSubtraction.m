% IncucytePreprocessNewer_FrameByFrameBackgroundSubtraction(FIPath,
% FICaliImg, BlankFIBackground, PCPath, "OTHERS")
% ---"OTHERS" (optional)---
...
function IncucytePreprocessNewer_FrameByFrameBackgroundSubtraction(...
    FIPath, FICaliImg, BlankFIBackground, PCPath, CompositePath, Skip, ...
    Delete, BitDepth, BackgroundDeviation)
    %% Parameter setting
        ...
        if ~ismember(i, Delete)
            if (BitDepth == 8)
                FIProcessed = uint8((double(imread(FIPath, i, 'Info', FIInfo)) ...
                    - BackgroundDeviation(i) * double(imread(BlankFIBackground, i))) ...
                    ./ FICaliImg);
